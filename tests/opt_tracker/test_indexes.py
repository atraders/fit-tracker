...
def test_str_to_datetime_index():
    assert indexes.str_to_datetime_index("ts1607727600.0") == datetime.fromtimestamp(
        1607727600.0
    )
    with pytest.raises(AssertionError):
...
def test_datetime_index_inversion():
    for timestamp in [1607727600.0, 486252000.0]:
        date = datetime.fromtimestamp(timestamp)
        assert (
            indexes.str_to_datetime_index(indexes.datetime_index_to_str(date)) == date
        )
    for ts_str in ["ts1607727600.0", "ts486252000.0"]:
        assert (
            indexes.datetime_index_to_str(indexes.str_to_datetime_index(ts_str))
            == ts_str
        )
